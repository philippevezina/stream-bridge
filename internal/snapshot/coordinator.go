...
func (c *Coordinator) EnsureConsistentRead(ctx context.Context) error {
	c.logger.Debug("Ensuring consistent read")
	...
	if _, err := c.conn.Execute("SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ"); err != nil {
		return fmt.Errorf("failed to set transaction isolation level: %w", err)
	}

	// Start a consistent read transaction
	if _, err := c.conn.Execute("START TRANSACTION WITH CONSISTENT SNAPSHOT"); err != nil {
		return fmt.Errorf("failed to start consistent snapshot transaction: %w", err)
	}

	c.logger.Debug("Consistent read transaction started")
	return nil
}

func (c *Coordinator) CommitConsistentRead(ctx context.Context) error {
	c.logger.Debug("Committing consistent read transaction")

	if c.conn == nil {
		return fmt.Errorf("no connection available")
	}

	if _, err := c.conn.Execute("COMMIT"); err != nil {
		return fmt.Errorf("failed to commit transaction: %w", err)
	}

	c.logger.Debug("Consistent read transaction committed")
	return nil
}
